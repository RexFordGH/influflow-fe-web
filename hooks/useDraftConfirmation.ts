...
export const useDraftConfirmation = () => {
  ...
  const { state, generateDraft, optimizeDraft, addMessage, clearState, confirmDraft, skipDraft } = context;
  ...
  const isReadyToConfirm = useMemo(() => {
    return state.draft !== null && !state.requiresReview;
  }, [state.draft, state.requiresReview]);
  ...
  const sendMessage = useCallback(async (userInput: string) => {
    ...
    if (!state.threadId) {
      // 如果没有 threadId，说明是第一次生成
      await generateDraft(userInput);
    } else {
      // 如果有 threadId，说明是优化草案
      await optimizeDraft(userInput);
    }
  }, [canSendMessage, state.threadId, generateDraft, optimizeDraft]);
  ...
  const isWaitingForConfirmation = useMemo(() => {
    return state.draft !== null && state.requiresReview && !state.isLoading;
  }, [state.draft, state.requiresReview, state.isLoading]);
  ...
  const getPerformanceMetrics = useCallback(() => {
    return {
      ...
      assistantMessages: state.messages.filter(m => m.type === 'assistant').length,
      hasError: hasError,
