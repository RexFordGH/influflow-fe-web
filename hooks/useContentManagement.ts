...
import { GeneratedContent, MindmapEdgeData, MindmapNodeData } from '@/types/content';
...
interface UseContentManagementReturn {
  ...
  handleTweetContentChange: (tweetId: string, newContent: string) => Promise<void>;
...
export function useContentManagement({
  ...
  // 数据转换方法
  const convertToGeneratedContent = useCallback((data: Outline): GeneratedContent => {
    return convertAPIDataToGeneratedContent(data);
  }, []);
  
  // 更新本地状态
  const updateLocalState = useCallback((data: Outline) => {
    onContentUpdate?.(data);
    
    // 重新构建思维导图
    const { nodes: newNodes, edges: newEdges } = convertThreadDataToMindmap(data);
