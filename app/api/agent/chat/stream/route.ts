...
export async function GET(req: NextRequest) {
  try {
    ...
    const chatThreadId = searchParams.get('chat_thread_id');

    if (!chatThreadId) {
      return new Response('缺少 chat_thread_id 参数', { status: 400 });
    }
    ...
    const targetUrl = `${API_HOST}/api/agent/chat/stream?chat_thread_id=${chatThreadId}`;
