...
import { Button, Modal, ModalContent, ModalHeader, ModalBody, Card, CardBody, Chip } from '@heroui/react';
import { InformationCircleIcon, DocumentTextIcon, GlobeAltIcon, UserIcon } from '@heroicons/react/24/outline';
...
const getMockSources = (sectionId: string) => {
  const sourcesData: { [key: string]: Array<{
    ...
    'background-analysis': [
      { type: 'report', title: '2024年行业研究报告', description: '权威机构发布的最新行业分析报告', reliability: 95 },
      { type: 'data', title: '市场数据统计', description: '来自官方统计局的市场规模数据', reliability: 90 }
    ],
    'core-viewpoints': [
      { type: 'interview', title: '专家访谈记录', description: '与行业专家的深度访谈内容', reliability: 88 },
      { type: 'ai', title: 'AI知识整合', description: '基于大量文献的AI分析结果', reliability: 85 }
    ],
    'practical-methods': [
      { type: 'survey', title: '用户调研反馈', description: '1000+用户的实践经验总结', reliability: 92 },
      { type: 'report', title: '最佳实践案例集', description: '成功企业的实施经验汇总', reliability: 89 }
    ],
    'future-trends': [
      { type: 'report', title: '技术趋势预测报告', description: '知名咨询公司的未来趋势分析', reliability: 87 },
      { type: 'ai', title: 'AI趋势预测', description: '基于大数据的AI预测模型结果', reliability: 83 }
    ]
  };

  return sourcesData[sectionId] || [
    { type: 'ai', title: 'AI生成内容', description: '基于训练数据生成的综合性内容', reliability: 80 }
  ];
};
...
export function EnhancedMarkdownRenderer({ 
  ...
  const sections = useMemo(() => {
    ...
    lines.forEach((line, index) => {
      ...
      if (trimmedLine.startsWith('#')) {
        ...
        const text = trimmedLine.replace(/^#+\s*/, '').replace(/[🧵📊💡🔧🚀✨]/g, '').trim();
        
        // 查找映射ID
        const mappingId = Object.keys(headingMappings).find(key => 
  ...
  const renderSection = (section: MarkdownSection, index: number) => {
    const isHighlighted = highlightedSection === section.mappingId || highlightedSection === section.id;
    const baseClasses = "transition-all duration-200 p-3 rounded-lg relative group";
    ...
    const renderEmoji = (text: string) => {
      return text.replace(/[🧵📊💡🔧🚀✨]/g, (match) => 
        `<span class="text-lg mr-1">${match}</span>`
      );
    };
    ...
    switch (section.type) {
      case 'heading':
        ...
        return (
          ...
          >
            <HeadingTag 
              className={headingClasses[section.level as keyof typeof headingClasses] || headingClasses[6]}
      ...
      case 'list':
        ...
        return (
          ...
          >
            {isNumberedList ? (
              <ol className="space-y-2 mb-3 ml-4">
                {listItems.map((item, idx) => (
                  <li key={idx} className="text-gray-700 text-sm leading-relaxed list-decimal">
                    <span className="ml-2">{item.replace(/^\d+\.\s*/, '').replace(/^\*\*(.*?)\*\*/, '<strong class="font-semibold">$1</strong>')}</span>
                  </li>
                ))}
              </ol>
            ) : (
                  ...
                  <li key={idx} className="text-gray-700 flex items-start text-sm leading-relaxed">
                    ...
                    <span 
                      dangerouslySetInnerHTML={{ 
                        __html: item.replace(/^[-*]\s*/, '').replace(/\*\*(.*?)\*\*/g, '<strong class="font-semibold text-gray-900">$1</strong>')
                      }} 
                    />
                  </li>
                ))}
  ...
  const getSourceIcon = (type: string) => {
    switch (type) {
      ...
      case 'interview': return <UserIcon className="h-4 w-4" />;
      case 'data': return <GlobeAltIcon className="h-4 w-4" />;
      case 'survey': return <UserIcon className="h-4 w-4" />;
      case 'ai': return <div className="h-4 w-4 bg-gradient-to-r from-purple-500 to-blue-500 rounded-full" />;
      ...
      >
        <ModalContent>
          <ModalHeader className="flex flex-col gap-1">
            ...
            <p className="text-sm text-gray-500">该内容段落的信息来源和可靠性分析</p>
          </ModalHeader>
          <ModalBody className="pb-6">
            <div className="space-y-4">
              {selectedSourceSection && getMockSources(selectedSourceSection).map((source, index) => (
                <Card key={index} className="border border-gray-200">
                  <CardBody className="p-4">
                    <div className="flex items-start space-x-3">
                      <div className={`p-2 rounded-lg bg-${getSourceColor(source.type)}-100`}>
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <span className="text-xs font-medium text-green-600">{source.reliability}%</span>
                          </div>
                        </div>
...
function SourceButton({ 
  ...
  return (
    <Button
      ...
      size="sm"
      variant="light"
      className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-blue-50"
