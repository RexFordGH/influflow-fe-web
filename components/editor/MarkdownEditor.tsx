...
const EditorToolbar = ({ editor }: { editor: any }) => {
  ...
  return (
    <div className="border-b border-gray-200 dark:border-gray-600 p-2 flex items-center gap-1 flex-wrap bg-gray-50 dark:bg-gray-700">
      {/* 文本格式化 */}
...
const MarkdownEditor = () => {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: 'prose prose-lg dark:prose-invert max-w-none focus:outline-none text-gray-900 dark:text-gray-100 break-words',
  ...
  const convertNodeToMarkdown = (node: any): string => {
    ...
    switch (node.type) {
      ...
      case 'listItem':
        return node.content?.map((child: any) => convertNodeToMarkdown(child)).join('') || ''
        
      case 'blockquote':
        const quoteContent = node.content?.map((child: any) => convertNodeToMarkdown(child)).join('\n') || ''
  ...
  return (
    <div className="h-full flex flex-col bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg overflow-hidden">
      {/* 工具栏 */}
      ...
      <div className="flex-1 relative">
        <div className="absolute inset-0 overflow-y-auto overflow-x-hidden bg-white dark:bg-gray-800">
          <EditorContent 
            editor={editor}
            className="w-full prose prose-lg dark:prose-invert max-w-none"
          />
        </div>
      </div>
      
      {/* 状态栏 */}
      <div className="border-t border-gray-200 dark:border-gray-600 p-2 text-xs text-gray-500 dark:text-gray-400 flex justify-between items-center bg-gray-50 dark:bg-gray-700">
        <div>
          字符数: {markdown.length} | 
          单词数: {wordCount}
        </div>
