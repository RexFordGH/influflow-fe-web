...
const MindmapFlow = () => {
  ...
  const handleNodesChange = useCallback((changes: any[]) => {
    ...
    const hasNonDeleteChanges = changes.some(change => change.type !== 'remove')
  ...
  const handleEdgesChange = useCallback((changes: any[]) => {
    ...
    // 检查是否有边被删除
    const hasEdgeDeletion = changes.some(change => change.type === 'remove')
  ...
  const handleNodesDelete = useCallback((nodesToDelete: any[]) => {
    ...
    // 找到所有需要删除的节点（包括子节点）
    const getDescendantNodeIds = (nodeId: string, allEdges: any[]): string[] => {
    ...
    console.log('删除节点:', deletedNodeIds, '级联删除:', Array.from(allNodesToDelete))
    
    // 过滤掉被删除的节点
    const currentNodes = nodes.filter(node => !allNodesToDelete.has(node.id))
    ...
    // 同步到Markdown
    syncFlowToMarkdown()
  }, [nodes, edges, setNodes, setEdges, setFlowNodes, setFlowEdges, syncFlowToMarkdown])
  ...
  useEffect(() => {
    if (edges.length > 0 && nodes.length > 0) {
      ...
      const hasChanges = updatedEdges.some((edge, index) => 
        edge.style?.stroke !== edges[index]?.style?.stroke
      )
  ...
  useEffect(() => {
    if (flowNodes.length > 0) {
      // 暂时禁用自动布局，避免干扰层级结构
      console.log('FlowNodes updated:', flowNodes.length, '个节点，暂不自动布局')
      // setTimeout(() => {
      //   handleAutoLayout()
      // }, 300)
    }
      ...
      >
        ...
        <Panel position="bottom-left" className="text-sm text-gray-500">
          <div className="bg-white dark:bg-gray-800 p-2 rounded border">
            节点数: {nodes.length} | 连接数: {edges.length} | Store节点: {flowNodes.length}
            {selectedNodeId && (
