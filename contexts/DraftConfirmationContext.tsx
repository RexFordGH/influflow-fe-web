...
import { 
  DraftConfirmationContextType, 
  ChatMessage,
  GenerateDraftResponse 
} from '@/types/draft';
import { draftReducer, initialState } from '@/reducers/draftReducer';

const DraftConfirmationContext = createContext<DraftConfirmationContextType | undefined>(undefined);

export const DraftConfirmationProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // 生成初始草案
  const generateDraft = useCallback(async (userInput: string) => {
    ...
    try {
      ...
      dispatch({ 
        type: 'SET_ERROR', 
        payload: error?.message || 'Failed to generate draft. Please try again.',
      });
  ...
  // 优化草案
  const optimizeDraft = useCallback(async (userInput: string) => {
    if (!state.threadId) {
    ...
    // 检查是否是确认意图
    const confirmationKeywords = ['ok', 'OK', '确认', '通过', 'approved', 'confirm', '好的', '可以'];
    ...
    try {
      ...
      // 根据响应确定消息内容
      let aiMessageContent = 'Here is your new overview of the post to be generated.';
      if (isConfirmation && !response.requires_review) {
        aiMessageContent = 'Great! Your draft has been confirmed. Generating your content now...';
      }
      ...
      dispatch({ 
        type: 'SET_ERROR', 
        payload: error?.message || 'Failed to optimize draft. Please try again.',
      });
      ...
      const errorMessage: ChatMessage = {
        id: uuidv4(),
        type: 'assistant',
        content: 'Sorry, there was an error optimizing the draft. Please try again.',
        timestamp: new Date(),
        status: 'error',
      };
      dispatch({ type: 'ADD_MESSAGE', payload: errorMessage });
...
export const useDraftConfirmationContext = () => {
  ...
  if (!context) {
    throw new Error('useDraftConfirmationContext must be used within DraftConfirmationProvider');
  }
  return context;
};